...
class World:
    ...
    def step(self, forward: float, turn: float, escape: float, dt: float):
        ...
        feeding = food is not None and food_distance < 0.48
        if feeding:
            fly.state = "feeding"
            fly.feeding_time += dt
        elif escape > 0.18:
            ...
            fly.state = "exploring"
            fly.feeding_time = 0.0
        fly.heading = (fly.heading + max(-1.0, min(1.0, turn)) * 2.2 * dt) % (2 * math.pi)
        target_speed = 0.03 if feeding else 0.25 + 0.9 * max(0.0, forward) + 0.8 * max(0.0, escape)
        ...
        radius = 0.16
        if not self.wall(nx + math.copysign(radius, dx or 1), fly.y):
            fly.x = nx
        else:
            fly.heading += 0.55
        if not self.wall(fly.x, ny + math.copysign(radius, dy or 1)):
            fly.y = ny
        else:
            fly.heading -= 0.55
        ...
        for food in list(self.foods):
            if math.hypot(food.x - fly.x, food.y - fly.y) < food.radius + 0.22 and fly.feeding_time > 1.0:
                ...
                fly.feeding_time = 0.0
        if not self.trail or math.hypot(fly.x - self.trail[-1][0], fly.y - self.trail[-1][1]) > 0.06:
            self.trail.append((fly.x, fly.y))
